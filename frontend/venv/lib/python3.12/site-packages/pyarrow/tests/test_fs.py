...
def test_s3_real_aws_region_selection():
    ...
    fs, path = FileSystem.from_uri('s3://mf-nwp-models/README.md.txt')
    ...
    fs, path = FileSystem.from_uri(
        's3://mf-nwp-models/README.md.txt?region=us-east-2')
    assert fs.region == 'us-east-2'
...
def test_s3_finalize():
    ...
    code = """if 1:
        ...
        with pytest.raises(ValueError, match="S3 .* finalized"):
            FileSystem.from_uri('s3://mf-nwp-models/README.md.txt')
        """
...
def test_concurrent_s3fs_init():
    ...
    code = """if 1:
        ...
        from pyarrow.fs import (FileSystem, S3FileSystem,
                                ensure_s3_initialized, finalize_s3)
        threads = []
        fn = lambda: FileSystem.from_uri('s3://mf-nwp-models/README.md.txt')
        for i in range(4):
